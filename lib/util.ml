  ...
  struct
    ...
    let rec receive_message fd contents =
      let len = Unix.recv fd buf 0 buf_len [] in
        if len = 0 then Bytes.concat Bytes.empty (List.rev contents)
        else receive_message fd ((Bytes.sub buf 0 len)::contents)
    ;;
    ...
    let inet_fetch_url link : string option =
      ...
      in
      let i = Unix.send fd msg 0 (Bytes.length msg) [] in
        if i = -1 then raise (Failure "Unix.send failed") else
          let result = Bytes.to_string (receive_message fd []) in
    ...
    let file_fetch_url link : string option =
      ...
      let rec receive_message chan contents =
        let len = input chan buf 0 buf_len in
          if len = 0 then Bytes.concat Bytes.empty (List.rev contents)
          else receive_message chan ((Bytes.sub buf 0 len)::contents) in
      let result = receive_message chan [] in
        close_in chan ; Some (Bytes.to_string result)
