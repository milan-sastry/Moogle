...
let send_std_response client_fd =
  Unix.send client_fd (Bytes.of_string std_response) 0 (String.length std_response) []
;;
  ...
  in
    try
      ...
      let response =
        ...
        else if query_string = "data/PU-gle.jpg"
        then std_response_header "image/jpeg" ^ (read_page query_string)
